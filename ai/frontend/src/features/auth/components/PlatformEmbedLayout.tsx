...
import { authActions, AI_AUTH_STORAGE_KEY } from '@/features/auth/store/authSlice'
import { useRefreshAiRole } from '@/features/auth/hooks/useRefreshAiRole'
import { isAllowedParentOrigin } from '@/features/auth/utils/identityConfig'
import type { AiRole, UserProfile } from '@/features/auth/types/auth.types'

type PlatformEmbedLayoutProps = {
  parentOrigin: string
}

function roleFromClaims(platformRole?: PlatformRole): AiRole {
...
export function PlatformEmbedLayout({ parentOrigin }: PlatformEmbedLayoutProps) {
  ...
  const { isAwaitingToken } = usePlatformEmbedAuth({
    ...
    authStorageKey: AI_AUTH_STORAGE_KEY,
    onPersistedSession: handlePersistedSession,
  })

  useRefreshAiRole(isAwaitingToken)
