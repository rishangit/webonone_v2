...
import { fetchAiRole } from '@/features/auth/utils/fetchAiRole'
...
export function getAiRedirectUri(path = '/'): string {
...
export function usePlatformSessionBootstrap(): PlatformBootstrapState {
  ...
  const getRedirectUri = useCallback((path: string) => getAiRedirectUri(path), [])
  ...
  const onSuccess = useCallback(
    async (result: ExchangeAuthCodeResult) => {
      ...
      const role: AiRole = await fetchAiRole(result.accessToken)
