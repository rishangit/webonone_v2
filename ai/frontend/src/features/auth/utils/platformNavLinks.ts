...
export function appendPlatformQueryToLocalNav(
  ...
  return items.map((item) => {
    if (item.type === 'item') {
      return isLocalEmailPath(item.to) ? { ...item, to: appendQueryString(item.to, search) } : item
    }

    return {
      ...item,
      children: item.children.map((child) =>
        isLocalEmailPath(child.to) ? { ...child, to: appendQueryString(child.to, search) } : child,
      ),
    }
  })
}
