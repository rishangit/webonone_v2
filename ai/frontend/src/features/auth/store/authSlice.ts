...
import type { AiRole, UserProfile } from '../types/auth.types'
...
export const AI_AUTH_STORAGE_KEY = 'ai_auth'
const PLATFORM_STORAGE_KEY = 'ai_platform_context'

export function clearAiAuthStorage(): void {
  clearServiceAuthSession(AI_AUTH_STORAGE_KEY)
...
function loadStoredAuth(): Pick<AuthState, 'accessToken' | 'user'> {
  const stored = readServiceAuthSession<UserProfile>(AI_AUTH_STORAGE_KEY)
  if (!stored) {
...
function persistAuth(state: Pick<AuthState, 'accessToken' | 'user'>) {
  if (state.accessToken && state.user) {
    writeServiceAuthSession(AI_AUTH_STORAGE_KEY, {
...
export const authSlice = createSlice({
  ...
  reducers: {
    ...
    setUserRole(state, action: PayloadAction<AiRole>) {
      if (state.user) {
    ...
    profileFetchSucceeded(
      state,
      action: PayloadAction<{ role: AiRole; companyId?: string | null }>,
    ) {
      state.isProfileLoading = false
