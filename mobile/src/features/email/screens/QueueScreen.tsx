...
import { emailAdminApi, type EmailAdminQueueItem } from '@/shared/services/emailAdminApi'
...
export function QueueScreen() {
  ...
  const canRetry = user?.role === 'super_admin'
  const [busyId, setBusyId] = useState<string | null>(null)

  const list = useServerPaginatedList<EmailAdminQueueItem>({
    pageSize: 24,
    fetchPage: async (query) => {
      const result = await emailAdminApi.listQueue({
  ...
  async function retry(id: string) {
    setBusyId(id)
    try {
      await emailAdminApi.retryQueueItem(id)
      toast({ title: 'Email queued again' })
      list.reload()
    } catch (err) {
      toast({
        title: 'Retry failed',
