...
<#
.SYNOPSIS
  Stage Identity build output into this folder for IIS.

.DESCRIPTION
  Copies frontend dist and backend dist into identity/deploy so IIS physical path
  can point directly at identity\deploy.

  Runtime config: identity\backend\.env
  Runtime dependencies: repo root node_modules (from npm install at repo root)
  Preserves web.config and IIS.md. Run via: npm run deploy -w identity-root
...
if ($CleanOnly) {
    Write-Host 'Cleaning identity\deploy (keeping web.config, stage-deploy.ps1, IIS.md) ...'
    Clear-DeployGenerated
    Write-Host 'Done.'
    exit 0
}

$IdentityRoot = Split-Path $DeployDir -Parent
$FrontendDist = Join-Path $IdentityRoot 'frontend\dist'
$BackendDist = Join-Path $IdentityRoot 'backend\dist'

if (-not (Test-Path $FrontendDist)) {
    throw "Frontend build not found at $FrontendDist. Run npm run build -w identity-root first."
}
if (-not (Test-Path $BackendDist)) {
    throw "Backend build not found at $BackendDist. Run npm run build -w identity-root first."
}
...
Write-Host 'Staging Identity for IIS in identity\deploy ...'
...
Write-Host ''
Write-Host 'Runtime env: identity\backend\.env'
