...
export const TEMPLATE_FORM_DIALOG_SIZE = {
...
interface TemplateFormDialogProps {
  ...
  /** Called when a core-hosted dialog saves successfully (list refresh). */
  onHostedSaved?: () => void
  /** Render form body for `/embed/dialogs/...` (no CustomDialog chrome). */
...
export function TemplateFormDialog({
      ...
      <Form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
        ...
        <FormField label="HTML body" htmlFor="template-html" required error={fieldErrors.htmlBody}>
          <Textarea
            ...
            onChange={(e) => patchField('htmlBody', e.target.value)}
          />
        </FormField>
        ...
        >
          <Textarea
            ...
            onChange={(e) => patchField('textBody', e.target.value)}
          />
        </FormField>
  ...
  return (
    <CustomDialog
      ...
      title={copy.title}
      description={copy.description}
      sizeWidth={TEMPLATE_FORM_DIALOG_SIZE.sizeWidth}
      sizeHeight={TEMPLATE_FORM_DIALOG_SIZE.sizeHeight}
      footer={actions}
    >
      {formBody}
    </CustomDialog>
