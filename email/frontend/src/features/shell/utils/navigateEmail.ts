...
/**
 * List/detail navigation that updates the WebOnOne shell URL when Email is
 * embedded (`/email/templates/:id`). Standalone keeps peer paths.
 */
export function useNavigateEmail() {
  ...
  const goToList = useCallback(() => {
    if (parentOrigin) {
      sendPlatformNavigate(parentOrigin, '/email/templates')
      return
    }
  ...
  const goToDetail = useCallback(
    (id: string) => {
      if (parentOrigin) {
        sendPlatformNavigate(parentOrigin, `/email/templates/${id}`)
        return
      }
  ...
  const goToPreview = useCallback(
    (id: string) => {
      if (parentOrigin) {
        sendPlatformNavigate(parentOrigin, `/email/templates/${id}/preview`)
        return
      }
  ...
  const goToVersions = useCallback(
    (id: string) => {
      if (parentOrigin) {
        sendPlatformNavigate(parentOrigin, `/email/templates/${id}/versions`)
        return
      }
