    ...
    <div className="space-y-4">
      <div className="space-y-4 rounded-lg border border-[hsl(var(--glass-border))] bg-[hsl(var(--glass-bg))] p-4">
        ...
        <div className="space-y-1">
          <p className="text-xs font-medium text-muted-foreground">{t('variant.sku')}</p>
          <p className="text-sm text-foreground">{values.sku || t('noDescription')}</p>
        </div>
      </div>

      <div className="space-y-3 rounded-lg border border-[hsl(var(--glass-border))] bg-[hsl(var(--glass-bg))] p-4">
        <h4 className="text-sm font-medium text-foreground">{t('variant.attributeValues')}</h4>
        {attributeRows.length === 0 ? (
          <p className="text-sm text-muted-foreground">{t('variant.noValuesOnVariant')}</p>
        ) : (
