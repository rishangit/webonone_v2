...
function isLocalDataPath(to: string): boolean {
...
export function appendPlatformQueryToLocalNav(
  items: NavConfigItem[],
  search: string,
): NavConfigItem[] {
  if (!search) {
  ...
  return items.map((item) => {
    if (item.type === 'item') {
      return isLocalDataPath(item.to) ? { ...item, to: appendQueryString(item.to, search) } : item
    }

    return {
      ...item,
      children: item.children.map((child) =>
        isLocalDataPath(child.to) ? { ...child, to: appendQueryString(child.to, search) } : child,
      ),
    }
  })
}
