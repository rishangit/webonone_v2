...
<#
.SYNOPSIS
  Stage Email build output into this folder for IIS.

.DESCRIPTION
  Copies frontend dist and backend dist into email/deploy so IIS physical path
  can point directly at email\deploy.

  Runtime config: email\backend\.env
  Runtime dependencies: repo root node_modules (from npm install at repo root)
  Preserves web.config and IIS.md. Run via: npm run deploy -w email-root
...
if ($CleanOnly) {
    Write-Host 'Cleaning email\deploy (keeping web.config, stage-deploy.ps1, IIS.md) ...'
    Clear-DeployGenerated
    Write-Host 'Done.'
    exit 0
}

$EmailRoot = Split-Path $DeployDir -Parent
$FrontendDist = Join-Path $EmailRoot 'frontend\dist'
$BackendDist = Join-Path $EmailRoot 'backend\dist'

if (-not (Test-Path $FrontendDist)) {
    throw "Frontend build not found at $FrontendDist. Run npm run build -w email-root first."
}
if (-not (Test-Path $BackendDist)) {
    throw "Backend build not found at $BackendDist. Run npm run build -w email-root first."
}
...
Write-Host 'Staging Email for IIS in email\deploy ...'
...
Write-Host ''
Write-Host 'Runtime env: email\backend\.env'
