...
import type { PaymentRole, UserProfile } from '../types/auth.types'
...
export const PAYMENT_AUTH_STORAGE_KEY = 'payment_auth'
const PLATFORM_STORAGE_KEY = 'payment_platform_context'

export function clearPaymentAuthStorage(): void {
  clearServiceAuthSession(PAYMENT_AUTH_STORAGE_KEY)
...
function loadStoredPlatform(): PlatformContext {
  try {
    ...
    return {
      ...
      coreNavVariant: parsed.coreNavVariant ? parsePlatformNavVariant(parsed.coreNavVariant) : null,
    }
  } catch {
    return { returnUrl: null, coreNavVariant: null }
  }
  ...
  if (platform.returnUrl) {
    sessionStorage.setItem(
      ...
      JSON.stringify({
        ...
        coreNavVariant: platform.coreNavVariant ? toCoreNavQueryValue(platform.coreNavVariant) : null,
      }),
    )
  } else {
    sessionStorage.removeItem(PLATFORM_STORAGE_KEY)
...
function loadStoredAuth(): Pick<AuthState, 'accessToken' | 'user'> {
  const stored = readServiceAuthSession<UserProfile>(PAYMENT_AUTH_STORAGE_KEY)
  if (!stored) {
...
function persistAuth(state: Pick<AuthState, 'accessToken' | 'user'>) {
  if (state.accessToken && state.user) {
    writeServiceAuthSession(PAYMENT_AUTH_STORAGE_KEY, {
...
export const authSlice = createSlice({
  ...
  reducers: {
    ...
    setUserRole(state, action: PayloadAction<PaymentRole>) {
      if (state.user) {
        state.user.role = action.payload
        persistAuth(state)
      }
    },
    setUserCompanyId(state, action: PayloadAction<string | null | undefined>) {
      if (state.user) {
        state.user.companyId = action.payload ?? null
        persistAuth(state)
      }
    },
    logout(state) {
    ...
    profileFetchSucceeded(
      state,
      action: PayloadAction<{ role: PaymentRole; companyId?: string | null }>,
    ) {
      state.isProfileLoading = false
