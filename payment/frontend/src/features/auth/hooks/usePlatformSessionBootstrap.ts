...
import { fetchPaymentRole } from '@/features/auth/utils/fetchPaymentRole'
...
export function usePlatformSessionBootstrap(): PlatformBootstrapState {
  ...
  const getRedirectUri = useCallback((path: string) => getPaymentRedirectUri(path), [])
  ...
  const onSuccess = useCallback(
    async (result: ExchangeAuthCodeResult) => {
      ...
      const role: PaymentRole = await fetchPaymentRole(result.accessToken)
