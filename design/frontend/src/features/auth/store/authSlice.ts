...
import type { DesignRole, UserProfile } from '../types/auth.types'
...
export const DESIGN_AUTH_STORAGE_KEY = 'design_auth'
const PLATFORM_STORAGE_KEY = 'design_platform_context'

export function clearDesignAuthStorage(): void {
  clearServiceAuthSession(DESIGN_AUTH_STORAGE_KEY)
...
function loadStoredAuth(): Pick<AuthState, 'accessToken' | 'user'> {
  const stored = readServiceAuthSession<UserProfile>(DESIGN_AUTH_STORAGE_KEY)
  if (!stored) {
...
function persistAuth(state: Pick<AuthState, 'accessToken' | 'user'>) {
  if (state.accessToken && state.user) {
    writeServiceAuthSession(DESIGN_AUTH_STORAGE_KEY, {
...
export const authSlice = createSlice({
  ...
  reducers: {
    ...
    setUserRole(state, action: PayloadAction<DesignRole>) {
      if (state.user) {
    ...
    profileFetchSucceeded(
      state,
      action: PayloadAction<{ role: DesignRole; companyId?: string | null }>,
    ) {
      state.isProfileLoading = false
