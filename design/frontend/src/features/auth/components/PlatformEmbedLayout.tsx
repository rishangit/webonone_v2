...
import { authActions, DESIGN_AUTH_STORAGE_KEY } from '@/features/auth/store/authSlice'
import { useRefreshDesignRole } from '@/features/auth/hooks/useRefreshDesignRole'
import { isAllowedParentOrigin } from '@/features/auth/utils/identityConfig'
import type { DesignRole, UserProfile } from '@/features/auth/types/auth.types'

type PlatformEmbedLayoutProps = {
  parentOrigin: string
}

function roleFromClaims(platformRole?: PlatformRole): DesignRole {
...
export function PlatformEmbedLayout({ parentOrigin }: PlatformEmbedLayoutProps) {
  ...
  const { isAwaitingToken } = usePlatformEmbedAuth({
    ...
    authStorageKey: DESIGN_AUTH_STORAGE_KEY,
    onPersistedSession: handlePersistedSession,
  })

  useRefreshDesignRole(isAwaitingToken)
