...
<#
.SYNOPSIS
  Stage Support build output into this folder for IIS.

.DESCRIPTION
  Copies frontend dist and backend dist into support/deploy so IIS physical path
  can point directly at support\deploy.

  Runtime config: support\backend\.env
  Runtime dependencies: repo root node_modules (from npm install at repo root)
  Preserves web.config and IIS.md. Run via: npm run deploy -w support-root
...
if ($CleanOnly) {
    Write-Host 'Cleaning support\deploy (keeping web.config, stage-deploy.ps1, IIS.md) ...'
    Clear-DeployGenerated
    Write-Host 'Done.'
    exit 0
}

$SupportRoot = Split-Path $DeployDir -Parent
$FrontendDist = Join-Path $SupportRoot 'frontend\dist'
$BackendDist = Join-Path $SupportRoot 'backend\dist'

if (-not (Test-Path $FrontendDist)) {
    throw "Frontend build not found at $FrontendDist. Run npm run build -w support-root first."
}
if (-not (Test-Path $BackendDist)) {
    throw "Backend build not found at $BackendDist. Run npm run build -w support-root first."
}
...
Write-Host 'Staging Support for IIS in support\deploy ...'
...
Write-Host ''
Write-Host 'Runtime env: support\backend\.env'
