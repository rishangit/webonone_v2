...
import { Alert, AlertDescription, Button, LoadingState, PageShell } from '@webonone/ui-kit'
import { useAppDispatch } from '@/app/store/hooks'
import { authActions } from '@/features/auth/store/authSlice'
import type { UserProfile } from '@/features/auth/types/auth.types'
import { consumeAuthState } from '@/features/auth/utils/buildIdentityLoginUrl'
import { getAuthCallbackUrl, getIdentityApiBase } from '@/features/auth/utils/identityConfig'
...
export function AuthCallbackPage() {
  ...
  useEffect(() => {
    ...
    if (!code || !state) {
      setError(t('authCallbackMissing'))
      return
    }
    ...
    if (!stored) {
      setError(t('authCallbackInvalid'))
      return
    }
    ...
    fetch(`${getIdentityApiBase()}/auth/exchange`, {
      ...
      .then(async (res) => {
        ...
        navigate(stored.returnPath || '/feedback', { replace: true })
      })
      .catch((err: Error) => {
