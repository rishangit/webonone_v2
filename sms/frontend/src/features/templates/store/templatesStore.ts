...
} from '@/shared/services/smsApi'
import type { SmsTemplate, TemplatePreviewResult, TemplateVersion } from '@/shared/types/sms.types'
...
export const templatesSlice = createSlice({
  ...
  reducers: {
    ...
    loadListSucceeded(state, action: PayloadAction<SmsTemplate[]>) {
    ...
    fetchDetailSucceeded(state, action: PayloadAction<SmsTemplate>) {
    ...
    createSucceeded(state, action: PayloadAction<SmsTemplate>) {
    ...
    updateSucceeded(state, action: PayloadAction<SmsTemplate>) {
      state.detail = action.payload
      state.detailLastFetchedAt = Date.now()
      state.detailStatus = 'idle'
      state.lastFetchedAt = null
    ...
    setActiveSucceeded(state, action: PayloadAction<SmsTemplate>) {
    ...
    deleteRequested(state, action: PayloadAction<{ id: string }>) {
      state.deletingId = action.payload.id
      state.listError = null
    },
    deleteSucceeded(state, action: PayloadAction<{ id: string }>) {
    ...
    restoreVersionSucceeded(state, action: PayloadAction<SmsTemplate>) {
      state.detail = action.payload
      state.detailLastFetchedAt = Date.now()
      state.detailStatus = 'idle'
      state.lastFetchedAt = null
...
const loadListEpic: Epic = (action$, state$) =>
  action$.pipe(
    ...
    exhaustMap(() =>
      from(smsApi.listTemplates()).pipe(
...
const fetchDetailEpic: Epic = (action$, state$) =>
  action$.pipe(
    ...
    switchMap(([action]) => {
      ...
      return from(smsApi.getTemplate(id)).pipe(
...
const loadVersionsEpic: Epic = (action$, state$) =>
  action$.pipe(
    ...
    filter(([action, state]) => {
      ...
      const feature = (state as unknown as { templates: TemplatesState }).templates
      if (payload.force) return true
      return !(feature.versionsTemplateId === payload.id && isFresh(feature.versionsLastFetchedAt))
    }),
    debounceTime(300),
    switchMap(([action]) => {
      const { id } = (action as ReturnType<typeof templatesActions.loadVersionsRequested>).payload
      return from(smsApi.listTemplateVersions(id)).pipe(
...
const createEpic: Epic = (action$) =>
  action$.pipe(
    ...
    exhaustMap((action) => {
      ...
      return from(smsApi.createTemplate(body)).pipe(
...
const updateEpic: Epic = (action$) =>
  action$.pipe(
    ...
    exhaustMap((action) => {
      ...
      return from(smsApi.updateTemplate(id, body)).pipe(
...
const reloadVersionsAfterRestoreEpic: Epic = (action$) =>
  action$.pipe(
    ...
    map((action) => {
      const template = (action as ReturnType<typeof templatesActions.restoreVersionSucceeded>)
        .payload
      return templatesActions.loadVersionsRequested({ id: template.id, force: true })
    }),
  )
...
const setActiveEpic: Epic = (action$) =>
  action$.pipe(
    ...
    exhaustMap((action) => {
      ...
      return from(smsApi.setTemplateActive(id, isActive)).pipe(
...
const deleteEpic: Epic = (action$) =>
  action$.pipe(
    ofType(templatesActions.deleteRequested.type),
    exhaustMap((action) => {
      const { id } = (action as ReturnType<typeof templatesActions.deleteRequested>).payload
      return from(smsApi.deleteTemplate(id)).pipe(
        map(() => templatesActions.deleteSucceeded({ id })),
        catchError((err: Error) => of(templatesActions.deleteFailed(err.message))),
      )
    }),
  )
...
const restoreVersionEpic: Epic = (action$) =>
  action$.pipe(
    ...
    exhaustMap((action) => {
      ...
      return from(smsApi.restoreTemplateVersion(id, versionId)).pipe(
...
const previewEpic: Epic = (action$) =>
  action$.pipe(
    ...
    switchMap((action) => {
      const { id, payload } = (action as ReturnType<typeof templatesActions.previewRequested>)
        .payload
