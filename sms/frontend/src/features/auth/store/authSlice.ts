...
import type { SmsRole, UserProfile } from '../types/auth.types'
...
export const SMS_AUTH_STORAGE_KEY = 'sms_auth'
const PLATFORM_STORAGE_KEY = 'sms_platform_context'

export function clearSmsAuthStorage(): void {
  clearServiceAuthSession(SMS_AUTH_STORAGE_KEY)
...
function loadStoredAuth(): Pick<AuthState, 'accessToken' | 'user'> {
  const stored = readServiceAuthSession<UserProfile>(SMS_AUTH_STORAGE_KEY)
  if (!stored) {
...
function persistAuth(state: Pick<AuthState, 'accessToken' | 'user'>) {
  if (state.accessToken && state.user) {
    writeServiceAuthSession(SMS_AUTH_STORAGE_KEY, {
...
export const authSlice = createSlice({
  ...
  reducers: {
    ...
    setUserRole(state, action: PayloadAction<SmsRole>) {
      if (state.user) {
    ...
    profileFetchSucceeded(state, action: PayloadAction<SmsRole>) {
      state.isProfileLoading = false
