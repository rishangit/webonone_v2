...
import { fetchSmsRole } from '@/features/auth/utils/fetchSmsRole'
...
export function getSmsRedirectUri(path = '/'): string {
...
export function usePlatformSessionBootstrap(): PlatformBootstrapState {
  ...
  const getRedirectUri = useCallback((path: string) => getSmsRedirectUri(path), [])
  ...
  const onSuccess = useCallback(
    async (result: ExchangeAuthCodeResult) => {
      ...
      const role: SmsRole = await fetchSmsRole(result.accessToken)
