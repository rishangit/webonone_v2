# Host SMS on IIS — manual steps

Host **sms.webonone.com** with IIS physical path set to **`sms\deploy`**.

Run **`npm run deploy:sms`** when you are ready to publish. That builds the app and stages output into `sms\deploy\`. Normal development (`npm run dev:sms`) and compile-only builds (`npm run build:sms`) do **not** touch this folder.

```text
sms\deploy\              ← IIS physical path (generated on deploy)
...
sms\backend\.env         ← runtime secrets (read directly by Node on IIS)
node_modules\              ← at repo root (npm install); Node resolves deps from here
```

Committed files in `sms\deploy\`: **`web.config`**, **`IIS.md`** only.
...
This allows SMS login callbacks on `https://sms.webonone.com/callback`. After updating Identity env, run `npm run deploy:identity` and recycle the Identity app pool.
...
4. **MySQL** — create database `webonone_sms` and a user with access to it

---

## Step 2 — DNS and certificate

1. Add DNS record: **sms.webonone.com** → server IP (`A` or `CNAME`)
2. In IIS, have a TLS certificate that covers **sms.webonone.com** (or `*.webonone.com`)
...
```powershell
cd C:\Projects\webonone_v2\sms
npm run stage:deploy
```

To remove generated output during development (keeps only `web.config`, `IIS.md`):
...
3. **Site name:** `SMS`
4. **Physical path:** `C:\Projects\webonone_v2\sms\deploy`
5. **Binding:** Type `https`, Host name `sms.webonone.com`, select your certificate
6. Click **OK**

### Application pool

1. Select the **SMS** site → **Basic Settings** → note the app pool name
...
Grant the app pool identity **Read & execute** on `sms\deploy` (including subfolders), **Read** on `sms\backend\.env`, and **Read & execute** on the repo root `node_modules\`.

Typical identity: `IIS AppPool\SMS` (if pool name is `SMS`).

The app pool also needs **Write** on `sms\deploy\logs`.
...
| URL | Expected |
|-----|----------|
| `https://sms.webonone.com/api/v1/health` | `{"status":"ok","service":"sms"}` |
| `https://sms.webonone.com/` | SMS admin UI loads |
| Login via dashboard | Redirects to Identity, returns to SMS |

If the site fails, check `sms\deploy\logs\` for Node errors.
...
| Page spins / never loads (0 bytes, timeout) | **Do not hardcode `PORT=4016` in `web.config`.** Use `<environmentVariable name="PORT" value="%HTTP_PLATFORM_PORT%" />` |
| 500 / Node crash | Check `sms\deploy\logs\` and `sms\backend\.env` (DB credentials, `JWT_SECRET`) |
| SPA 404 | Confirm `sms\deploy\public\index.html` exists — run `npm run deploy:sms` |
...
Env templates live in **`sms\backend\.env.example`** and **`sms\frontend\.env.example`**.
