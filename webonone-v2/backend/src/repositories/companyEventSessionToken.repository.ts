...
export async function listTokensForSession(
  ...
  return db<CompanyEventSessionTokenRow>('company_event_session_tokens')
    ...
    .orderBy([
      ...
      { column: 'call_order', order: 'asc' },

      { column: 'token_number', order: 'asc' },
...
export async function findFirstWaitingToken(
  ...
  return db<CompanyEventSessionTokenRow>('company_event_session_tokens as token')
    ...
    .orderBy([
      ...
      { column: 'token.call_order', order: 'asc' },

      { column: 'token.token_number', order: 'asc' },
...
export async function findLastCompletedToken(
  ...
  return db<CompanyEventSessionTokenRow>('company_event_session_tokens')
    ...
    .orderBy([
      ...
      { column: 'call_order', order: 'desc' },

      { column: 'token_number', order: 'desc' },
...
export async function updateTokenCallOrder(
  ...
  if (!updated) throw new Error('Failed to update session token call order')
...
export async function updateTokenCallOrders(

  updates: Array<{ id: string; callOrder: number }>,
  ...
  await db.transaction(async (trx) => {
    ...
    for (const update of updates) {
      ...
      await trx('company_event_session_tokens').where({ id: update.id }).update({

        call_order: update.callOrder,
