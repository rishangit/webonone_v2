...
/**
 * Active session tokens with a null workflow pointer cannot appear in per-step
 * queues. Backfill to check-in (not checked in) or first space step (checked in).
 */
...
export async function down(): Promise<void> {
  // Cannot distinguish null pointers that were backfilled from never-set rows.
}
